...
//                 // return slotStart.isSame(event.startTime); // Only show event at the start time slot
//                   return slotStart.isSame(event.startTime) || (slotStart.isAfter(event.startTime) && slotStart.isBefore(event.endTime));
...
// //         }}
// //     >
// //         Booked: {dayjs(bookedEvent.startTime).format("HH:mm")} - {dayjs(bookedEvent.endTime).format("HH:mm")}
// //     </div>
// // )}
...
export default function Calendar({ selectedDate }) {
    ...
    // Function to handle manual event creation
    const handleCreateEvent = () => {
        setTempEvent({
            id: uuidv4(),
            title: "",
            startTime: dayjs(selectedDate).format("YYYY-MM-DDT00:00"),
            endTime: dayjs(selectedDate).format("YYYY-MM-DDT01:00"),
        });
        setIsModalOpen(true);
    };

    return (
        <div style={{ flex: 1, padding: "10px", userSelect: "none", overflowY: "auto", height: "100vh", maxWidth: "100%", boxSizing: "border-box" }}>
            {/* Create Event Button */}
            <button
                style={{
                    padding: "10px",
